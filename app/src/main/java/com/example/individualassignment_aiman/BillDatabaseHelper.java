...
public class BillDatabaseHelper extends SQLiteOpenHelper {
    ...
    public long insertBill(String month, double unit, double total, double rebate, double finalCost) {
    ...
    public Cursor getAllBills() {
        return getReadableDatabase().rawQuery("SELECT * FROM " + TABLE + " ORDER BY id DESC", null);
    }

    public Cursor getBillById(int id) {
        return getReadableDatabase().rawQuery("SELECT * FROM " + TABLE + " WHERE id=?",
                new String[]{String.valueOf(id)});
    }

    public int updateBill(int id, String month, double unit, double total, double rebate, double finalCost) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(COL_MONTH, month);
        cv.put(COL_UNIT, unit);
